...
A Cloudflare Worker that scrapes Netflix's public jobs API on a daily cron, persists state in an R2 bucket, and serves a self-contained HTML page for browsing, filtering, and searching the listings. There is no test suite, no linter, no build step — `wrangler` handles bundling and deploy.
...
Sync is idempotent and incremental: each run diffs the listing against the cached store. New ids are added with `_status: "open"`; ids that disappear from the listing flip to `"removed"`; ids that reappear flip back to `"open"`. A job is re-enriched when `t_update > _enriched_at`.

## Non-obvious behaviors — preserve these

1. **`isUSARemote` (`src/parsers.ts`) checks the `locations` strings, not `work_location_option`.** The API mislabels USA-Remote jobs as a mix of `onsite` and `remote_local`, so the WLO field is unreliable. The only trustworthy signal is `"USA - Remote"` appearing in `locations`.
2. **`PAGE_SIZE = 10` in `src/netflix.ts`** is a hard cap from Netflix's API — passing larger `num` is silently truncated to 10. Don't "optimize" the pagination loop by raising it.
